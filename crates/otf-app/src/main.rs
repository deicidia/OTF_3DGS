...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec = rerun::RecordingStreamBuilder::new("OTF_3DGS").spawn()?;
    rec.set_duration_secs("stable_time", 0.0);
    const NUM_POINTS: usize = 100;

    let (points1, colors1) = color_spiral(NUM_POINTS, 2.0, 0.02, 0.0, 0.1);
    let (points2, colors2) = color_spiral(NUM_POINTS, 2.0, 0.02, std::f32::consts::TAU * 0.5, 0.1);
    ...
    for i in 0..400 {
        ...
        rec.log(
            ...
            &rerun::archetypes::Transform3D::from_rotation(rerun::RotationAxisAngle::new(
                glam::Vec3::Z,
                rerun::Angle::from_radians(time / 4.0 * std::f32::consts::TAU),
            )),
        )?;
    }
    Ok(())
